...
class Block(nn.Module):
    def __init__(self, n_ctx, config, scale=False, alpha=1.0):
        super().__init__()
    ...
    def forward(
        ...
        # residual connection
        hidden_states = attn_output + hidden_states * self.alpha
        ...
        if encoder_hidden_states is not None:
            ...
            # residual connection
            hidden_states = hidden_states * self.alpha + attn_output
            outputs = outputs + cross_attn_outputs[2:]  # add cross attentions if we output attention weights

        feed_forward_hidden_states = self.mlp(self.ln_2(hidden_states))
        # residual connection
        hidden_states = hidden_states * self.alpha + feed_forward_hidden_states
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config, norm=None):
        ...
        if norm == "deepnorm":
            if config.add_cross_attention:
                alpha = (3 * config.n_layer) ** (1 / 4)
            else:
                alpha = (2 * config.n_layer) ** (1 / 4)
        else:
            alpha = 1.0
        self.h = nn.ModuleList([Block(config.n_ctx, config, scale=True, alpha=alpha) for _ in range(config.n_layer)])
